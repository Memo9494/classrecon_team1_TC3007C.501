...
while True:
    ...
    for res in results:
        ...
        for xmin, ymin, xmax, ymax, track_id in tracks:
            ...
            # f_data_locations = face_recognition.face_locations(cut_frame)                                    # Obtiene las coordenadas del rostro en la imagen
            # print(f_data_locations)
            # if f_data_locations != []:                                                                   # Si se detecta un rostro
            #     print("[PROCESO] Rostros detectados")
            #     f_frame_codings = face_recognition.face_encodings(cut_frame,f_data_locations)                # Obtenemos las características del rostro encontrado
            ...
            #     cv2.rectangle(cut_frame, (left, top), (right, bottom), (0,255,0), 2)
            #     cv2.rectangle(cut_frame, (left, bottom -10), (right, bottom), (0,255,0), cv2.FILLED)
            #     font = cv2.FONT_HERSHEY_DUPLEX
            #     cv2.putText(cut_frame, name, (left +3, bottom -3), font, 0.4, (255,255,255), 1)
            # else:
