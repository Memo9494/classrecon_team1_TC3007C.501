...
with open(data_path, "r") as file:
    for line in file.readlines():
        ...
        alumni_list[num_alumni] = {"name":name, "attendance": 0, "delay":0, "participations":0, "track_id": None} # Dicicionario por alumno -> Nombre:[asistencia, conteo]
        num_alumni += 1
...
out = cv2.VideoWriter('output.avi', fourcc, 20.0, (640, 480))  # Nombre del archivo, codec, fps, tamaño del cuadro
...
while cap.isOpened():
    ...
    if (results[0] != []) and (results[0].boxes.id != None):
        ...
        for track_id, kpts in zip(track_ids, keypoints):
            ...
            if actual_time < time_lim_attendance:
                if frame_count % 12 == 0:
                    face_recog(face_frame, data_encodings, attendance_taken, alumni_list,track_id)
                for element, attributes in alumni_list.items():
                    track_id_ = attributes.get('track_id', None)  # Obtiene el valor de 'track_id' o None si no existe
                    if track_id == track_id_:
                        name = alumni_list[element]["name"]
                        break
                    else:
                        name = "Desconocido"
    ...
    # Escribe el fotograma en el objeto VideoWriter
    out.write(frame)
